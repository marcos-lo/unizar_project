...
-- Internal signals MIPS	
	...
	signal RW_EX, RW_MEM, RW_WB, Reg_Rs_ID, Reg_Rs_EX, Reg_Rt_ID, Reg_Rd_EX, Reg_Rt_EX, RW_EX_internal: std_logic_vector(4 downto 0);
...
begin
	...
	ALU_MIPs: ALU_Vector_MAC PORT MAP ( clk => clk, reset => reset_EX, valid_I_EX => valid_I_EX, DA => Mux_A_out, DB => ALU_Src_out, ALUctrl => ALUctrl_EX, Dout => ALU_out_EX, ready => ALU_ready, Exception_accepted => Exception_accepted, RTE_EX => RTE_EX);
	
	
	mux_dst: mux2_5bits port map (Din0 => Reg_Rt_EX, DIn1 => Reg_Rd_EX, ctrl => RegDst_EX, Dout => RW_EX_internal);
	RW_EX <= RW_EX_internal;
	reset_MEM <= (reset);
...
-------------------------------------------------------------------------------------------------- 
	...
	inc_cycles <= '1';--Done
-- Instrucciones completadas: Solo se cuenta si una instrucción VÁLIDA llega a la última etapa (WB)
	inc_I <= valid_I_WB; 

	-- Excepciones: Solo se cuenta cuando el gestor acepta la interrupción
	inc_Exceptions <= Exception_accepted;

	---------------------------------------------------------
	-- CONTADORES DE PARADAS (Respetando la jerarquía)
	---------------------------------------------------------
	-- 1. Paradas Estructurales (Las de mayor prioridad)
	-- Memoria: Si la etapa MEM es válida pero la memoria dice "no estoy lista"
	inc_Mem_stalls <= (not IO_MEM_ready) and valid_I_MEM; 

	-- ALU Multiciclo (MAC): Si la etapa EX es válida pero la ALU dice "no estoy lista"
	inc_EX_stalls <= (not ALU_ready) and valid_I_EX and (not inc_Mem_stalls);

	-- 2. Riesgos de Datos (Segunda prioridad)
	-- Si hay stall_ID, PERO nos aseguramos de no contarlo si ya estamos parados por la memoria o la ALU (stall_MIPS)
	inc_data_stalls <= stall_ID and not stall_MIPS; 

	-- 3. Riesgos de Control (Tercera prioridad)
	-- Cuando se toma un salto, Kill_IF se pone a '1'. Solo lo contamos en el ciclo que realmente avanza (no bloqueado por stall_MIPS)
	inc_control_stalls <= Kill_IF and not stall_MIPS;
